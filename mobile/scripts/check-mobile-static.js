...
checks.push(!missingApiCalls.length
  ...
  && appSource.includes('loadAuthenticatedData')
  && appSource.includes('lastBootstrapKey')
  && appSource.includes('Promise.allSettled')
  && appSource.includes('同步账号数据')
  && appSource.includes('getReviewTaskAnswer')
  && appSource.includes('variantQuestion?.correctAnswer')
  && appSource.includes('variantQuestion?.options')
  && clientSource.includes('consentAccepted = false')
  && clientSource.includes("gradeStage: 'junior'")
  && clientSource.includes('body: { answer }')
